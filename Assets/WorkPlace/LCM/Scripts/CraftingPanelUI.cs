...
public class CraftingPanelUI : MonoBehaviour
{
    ...
    [SerializeField] private GameObject _craftingPanel; // ??? ???? UI?? ??? ??? ?еш? GameObject (?? ???????? ???? GameObject ????? ?м╘??? ???? ????)
    ...
    [SerializeField] private Transform _materialListContainer; // ??? ????? ??? ?еш?
    ...
    private void Awake()
    {
        ...
        // Example: UI ???? ?? ??????
        // _craftingPanel.SetActive(false); // ???? ?? ???????? ?еш? ?м╘????? ???
        // _craftingDetailPanel.SetActive(false);

        // SampleCraftingUIController?? Awake ???? ????
        // ?? ???????? CraftingPanel ????? ??ве?? _craftingPanel ?????? this.gameObject?? ????? ?? ??????.
    ...
    private void DisplayRecipeDetails(Recipe recipe)
    {
        ...
        {
            string energyStatus = $"????? ???бд?: {recipe.energyCost}";
