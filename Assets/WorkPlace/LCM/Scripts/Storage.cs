...
public class Storage : Singleton<Storage>
{
    ...
    public void RemoveItem(Item itemData, int quantityToRemove)
    {
        ...
        int totalCountInStorage = GetItemCount(itemData);
        if (totalCountInStorage < quantityToRemove)
        {
            Debug.LogWarning($"창고에 '{itemData.itemName}'이(가) 부족하여 {quantityToRemove}개를 모두 제거할 수 없습니다. 보유: {totalCountInStorage}");
            quantityToRemove = totalCountInStorage; // 보유한 만큼만 제거
            if (quantityToRemove == 0) return; // 보유한 아이템이 없으면 바로 종료
        }

        List<InventorySlot> slotsToProcess = new List<InventorySlot>();
        foreach (var slot in storageSlots)
        {
        ...
        foreach (var slot in slotsToProcess)
        {
            if (slot.myItemData == itemData && slot.myItemUI != null)
            {
                ...
                OnStorageSlotItemUpdated?.Invoke(Array.IndexOf(storageSlots, slot), itemData, finalQuantityForEvent);
